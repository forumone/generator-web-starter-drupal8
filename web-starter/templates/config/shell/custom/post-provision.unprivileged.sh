...
if [[ -f "${DOCROOT}/sites/default/services.vm.yml" ]]; then
  if [[ ! -f "${DOCROOT}/sites/default/services.yml" || -w "${DOCROOT}/sites/default/services.yml" ]]; then
    echo 'Copying services file'
    cd ${DOCROOT}/sites/default && cp services.vm.yml services.yml
  fi
fi
